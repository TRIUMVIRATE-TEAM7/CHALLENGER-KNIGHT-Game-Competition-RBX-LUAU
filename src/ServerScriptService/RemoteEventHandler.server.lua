...
DashEvent.OnServerEvent:Connect(function(Player)
	if GetPlayerStates(Player, "Dashed") == nil then
		if not GetPlayerStates(Player, "Stunned") then
			...
			task.wait()
			local stunIndicator = ReplicatedStorage.VFX.PlayerVFX.Indicator.StunnedBillBoard:Clone()
			stunIndicator.TextLabel.Text = "[I cant move.]"
			stunIndicator.Parent = Player.Character.Head
			game:GetService("TweenService"):Create(stunIndicator.TextLabel, TweenInfo.new(0.5), {TextTransparency = 1} ):Play()
			Debris:AddItem(stunIndicator, 1)
		end
	end
end)
---------------------------------------------------
