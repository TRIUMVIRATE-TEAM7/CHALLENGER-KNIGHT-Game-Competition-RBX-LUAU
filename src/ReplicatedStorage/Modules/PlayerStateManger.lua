...
function module.PlayerStunned(Player, value, duration)
	if not Player or not Player.Character or not Player.Character:WaitForChild("Humanoid") then return end ------- MAKE SURE THE PLAYER IS SPAWNED IN (BAD THINGS HAPPEN IF WE DONT CHECK FOR THIS)
	...
	if value then
		states[Player]["Stunned"] = true
		
		local OriginalWalkSpeed = Player.Character.Humanoid.WalkSpeed
		local OriginalJumpPower = Player.Character.Humanoid.JumpPower
		
		local stunIndicator = game:GetService("ReplicatedStorage").VFX.PlayerVFX.Indicator.StunnedBillBoard:Clone()
		stunIndicator.Parent = Player.Character.Torso
		
		coroutine.wrap(function()
			wait(duration)
			game:GetService("TweenService"):Create(stunIndicator.TextLabel, TweenInfo.new(0.2), {TextTransparency = 1} ):Play()
			module.RemoveStates(Player, "Stunned")
